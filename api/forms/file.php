...
class CForm_file extends CForm_Text {
    ...
    function Add($name, $add, $multiple = false) {
        ...
        <div class="inner_foto">
            ...
            <input class="fileimg2" type="text" name=<?= $name ?>_MANUAL<? if ($multiple) { ?>[]<? }; ?> value="" /><a class="btn file_img_select fileimg2" style="margin-top:-10px;">Выбрать файл</a>			
        </div>
        <?
    }
    ...
    function Edit($name, $data, $add, $multiple = false) {
        ...
        <div class="inner_foto">
            ...
            <a class="btn file_img_select fileimg2" style="margin-top:-10px;">Выбрать файл</a>
        ...
        if ($multiple) {
            if (isset($_POST[$name . "_DELETE"][$_files_counter[$name]])) {
                ...
                if ($_POST[$name . "_OLD"][$_files_counter[$name]] != "")
                    @unlink(FOLDER_ROOT . $_POST[$name . "_OLD"][$_files_counter[$name]]);
            };

            if (isset($_POST[$name . "_MANUAL"][$_files_counter[$name]]))
                if (($_POST[$name . "_MANUAL"][$_files_counter[$name]]))
                    if (file_exists(FOLDER_ROOT . $_POST[$name . "_MANUAL"][$_files_counter[$name]])) {
